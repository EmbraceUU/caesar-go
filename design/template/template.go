...
func (d TimeDraw) checkParams(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "按时间抽奖类型:特殊参数校验...")
	return nil
}

func (d TimeDraw) getPrizesNode(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "do nothing(抽取该场次的奖品即可，无需其他逻辑)...")
	return nil
}
...
func (d TimesDraw) checkParams(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "按抽奖次数抽奖类型:特殊参数校验...")
	return nil
}

func (d TimesDraw) getPrizesNode(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "1. 判断是该用户第几次抽奖...")
	fmt.Println(components.runFuncName(), "2. 获取对应node的奖品信息...")
	fmt.Println(components.runFuncName(), "3. 复写原所有奖品信息(抽取该node节点的奖品)...")
	return nil
}
...
func (d AmountDraw) checkParams(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "按数额范围区间抽奖:特殊参数校验...")
	return nil
}

func (d AmountDraw) getPrizesNode(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "1. 判断属于哪个数额区间...")
	fmt.Println(components.runFuncName(), "2. 获取对应node的奖品信息...")
	fmt.Println(components.runFuncName(), "3. 复写原所有奖品信息(抽取该node节点的奖品)...")
	return nil
}
...
func (lottery *Lottery) checkSerialNo(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "校验活动编号(serial_no)是否存在、并获取活动信息...")
	return nil
}
...
func (lottery *Lottery) drawPrizes(ctx *TemplateContext) error {
	fmt.Println(components.runFuncName(), "抽奖...")
	return nil
}
